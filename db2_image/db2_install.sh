...
#Found in db2inst1's bashrc.
#Needed to prep the users env so we get the db2 cmd.
. /database/config/db2inst1/sqllib/db2profile

su - ${DB2INSTANCE?} -c 'db2 -vtf /docker-entrypoint-initdb.d/1-schema.sql'
su - ${DB2INSTANCE?} -c 'db2 -vtf /docker-entrypoint-initdb.d/2-insert-data.sql'
